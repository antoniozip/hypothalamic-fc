...
## Resolved 2026-09-19: the defect was a two-element buffer, not the lag convention

Both "next steps" above were wrong guesses. The correlogram lag convention and the J/Jmin
pairing were correct as written; the actual cause was that the port passed
`calc_log_post` a 100-element histogram where the reference passes a 102-element design
vector, the last two entries being the coupling sufficient statistics. Those two reads ran
off the end of the buffer (confirmed by AddressSanitizer), so the objective had no term
rewarding a positive coupling and LM drove every J to the -3 clamp.

Re-running this same script against the fixed build:

| implementation | density% | recall% | precis% | sign% | det.neg% | secs |
|---|---|---|---|---|---|---|
| C (`glmcc_fixed`) | 12.88 | 62.5 | 58.8 | **100.0** | 35.3 | 0.2 |
| Python (vendor) | 12.88 | 62.5 | 58.8 | **100.0** | 35.3 | 42.0 |

Identical to the digit, at 210x the speed. See `reports/glmcc_c_sign_bug.md`.

The conclusion that "every C-generated matrix in this repository is affected" still holds,
and those matrices have not yet been regenerated.
