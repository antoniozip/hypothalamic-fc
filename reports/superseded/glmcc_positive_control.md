...
## Superseded 2026-09-19 (twice)

These numbers came from a 1-indexed fixture (corrected in `reports/glmcc_python_vs_c.md`)
*and* from a C build with the sign defect. Both are fixed. Current results, same script,
same seed:

| scenario | spk/neuron | density% | recall% | precis% | sign% | det.neg% |
|---|---|---|---|---|---|---|
| generous | 36,448 | 8.16 | **92.5** | 52.1 | **100.0** | 57.7 |
| ongoing-like | 25,553 | 7.70 | **80.0** | 47.8 | **96.9** | 52.2 |
| lightON-like | 788 | 0.34 | 0.0 | 0.0 | — | 33.3 |

The headline conclusion of the original report — "the estimator does not recover couplings" —
was an artefact of the port. GLMCC recovers 80-92% of injected couplings with near-perfect
sign accuracy when given 25-36k spikes/neuron.

**The lightON regime is a real limitation, not a bug.** At 788 spikes/neuron the estimator now
detects almost nothing (3 detections, none true) rather than the earlier 50% "recall" at 0%
sign accuracy, which was spurious. Any lightON connectivity result rests on essentially no
statistical power.
