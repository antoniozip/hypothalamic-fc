...
# conn_ongoing_day = conn_ongoing_day[c(-1, -4, -9), c(-1, -4, -9)]
# conn_ongoing_night = conn_ongoing_night[c(-1, -4, -9), c(-1, -4, -9)]
# conn_evoked_day = conn_evoked_day[c(-1, -4, -9), c(-1, -4, -9)]
# conn_evoked_night = conn_evoked_night[c(-1, -4, -9), c(-1, -4, -9)]
...
png(file = paste0("/home/antonio/Documents/tim_brown/GLMCC/aggregate_circos_all_region_reduction.png"), units = "in",  width = 24, height = 24, res = 1200)
ref_conn = conn_ongoing_day
...
ref_conn = conn_ongoing_day - conn_ongoing_night
circos.clear()
sectors = factor(all_region_labels, levels = all_region_labels)
circos.par(cell.padding = c(0.02, 0, 0.02, 0))
circos.initialize(sectors, xlim = c(0, 10))
...
ref_conn = conn_evoked_day - conn_evoked_night
circos.clear()
sectors = factor(all_region_labels, levels = all_region_labels)
circos.par(cell.padding = c(0.02, 0, 0.02, 0))
circos.initialize(sectors, xlim = c(0, 10))
...
conn = read_xlsx("/home/antonio/Documents/allen_mouse_connectivity/conn_matrix.xlsx")
...
sub_conn = as.matrix(conn[c(207,208,17,42,145,128),c(207,208,17,42,145,128)]) 
hypothalamus_net <- network(sub_conn)
labs = c("VM", "VMH", "ARH", "DMH", "PVH","PH")
png(file = paste0("/home/antonio/Documents/tim_brown/GLMCC/aggregate_circos_all_region_reduction_structural_conn.png"), units = "in",  width = 24, height = 24, res = 1200)
plot.network(hypothalamus_net, label = labs, vertex.cex=4, label.pos=5, edge.lwd = sub_conn*80, arrowhead.cex = 3, usecurve = TRUE, edge.curve = 0.02)
dev.off()
